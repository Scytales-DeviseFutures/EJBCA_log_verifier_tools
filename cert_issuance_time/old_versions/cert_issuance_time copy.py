        ...
        for user_id, logs in user_logs.items():
            for log_index, log_entries in logs.items():
                ...
                if ca_userauth_time:
                    ...
                    file.write(f"{user_id}{sep}")
                    file.write(f"{cert_request_diff}{sep}")
                    file.write(f"{cert_stored_diff}{sep}")
                    file.write(f"{cert_creation_diff}{sep}")
                    file.write(f"{status_generated_diff}\n")

            
def write_statistics_to_file(user_logs, output_file):
    certs_processed = 0
    min_time = float('inf')
    max_time = float('-inf')
    total_time = 0
    ...
    with open(output_file, 'a') as file:
        for user_id, logs in user_logs.items():
            for log_index, log_entries in logs.items():
                ...
                if ca_userauth_time and status_generated_time:
                    certs_processed += 1
        ...
        avg_time = total_time / certs_processed if certs_processed > 0 else 'N/A'
        file.write(f"\nCerts;min;max;avg\n")
        file.write(f"{certs_processed};{min_time if certs_processed > 0 else 'N/A'};{max_time if certs_processed > 0 else 'N/A'};{avg_time}\n")
...
def main():
    ...
    write_statistics_to_file(user_logs, args.output_file)
