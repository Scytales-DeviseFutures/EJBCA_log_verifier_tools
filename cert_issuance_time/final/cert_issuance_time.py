...
def mark_time(label):
    time_points[label] = time.time()

def print_time_differences():
    prev_time = None
    for label, timestamp in time_points.items():
        if prev_time is not None:
            time_diff = timestamp - prev_time
            print(f"Tempo decorrido desde '{prev_label}' até '{label}': {time_diff:.6f} segundos")
        prev_time = timestamp
        prev_label = label

def extract_user_logs(log_file):
    user_logs = defaultdict(lambda: defaultdict(list))
    current_log_set = defaultdict(int)
...
def write_output_to_file(user_logs, output_file, sep):
    with open(output_file, 'w') as file:
        file.write(f"User{sep}Request{sep}Stored{sep}Creation{sep}Generated\n")
